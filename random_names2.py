...
recnum = ['unassigned', 'unassigned', 'unassigned', 'unassigned', 'unassigned', \
          'unassigned', 'unassigned', 'unassigned', 'unassigned', ]
# prevyear = [6,4,8,1,2,3,5,0,7] #2015
prevyear = [7,5,6,0,1,8,3,2,4] #2016

def print_result(give, receive, lastyear):
  print ("Giver     \tReceiver     \tLast Year")
  print ("-----     \t--------     \t---------")
  i = 0
  for i in range(9):
    print (give[i], "    \t", give[receive[i]], "   \t", give[lastyear[i]])
    i = i + 1
  print ()
  ...
  while i < 9:
    ...
    while not new:
      ...
      if (loopcount == 200): 
        ...
        loopcount = 0
#        print_result(givers, recnum, prevyear)
        break
